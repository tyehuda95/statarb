...
Will try to make this into a bash script that can be run
...
import pandas as pd
import argparse
import glob #  iterate through files
...
def get_dates(input_folder):
    ...
    except Exception: # if there is any error do this - assumes no data in folder
...
"""
    ...
    parser.add_argument('-F', '--listF',  help = 'list of extra bloomberg functions apart from price, volume, ebitda, enterprise value and asset value')
    ...
    # convert data into bloomberg terminology
    ...
    function_list =  ['LAST_PRICE','PX_OPEN','VOLUME','AVG_PX','PX_HIGH','PX_LOW','PX_ASK','PX_BID','LAST_PRICE','TO_EBITDA','PE_RATIO','ENTERPRISE_VALUE','ASSET_VAL']+ extra_functions
